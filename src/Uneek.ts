...
export function VectorPairOf<A, B>(
  a: VectorOf<A>,
  b: VectorOf<B>,
  def_a: A,
  def_b: B
): VectorOf<Pair<A, B>> {
  return Utils.record_create(Utils.keysof(a, b), k =>
    Pair(Utils.or(a[k], def_a), Utils.or(b[k], def_b))
  )
}
...
export function FromXML(d: Document | string): Text {
  ...
  const sentenceparser = (s: Element) => {
    ...
    Array.from(s.getElementsByTagName('*')).map(w =>
      Array.from(w.attributes).map(x => bump2(struct, x.nodeName, x.value))
    )
...
export const flat = (v: VectorPair) =>
  Utils.record_traverse(v, ({a, b}, word) => ({a, b, word})).sort((l, r) => r.a + r.b - (l.a + l.b))

export const full = (a: Text, b: Text) =>
  Utils.flatten(
    Utils.keysof(a, b)
      .sort()
      .map(key => flat(VectorPair(a[key] || {}, b[key] || {})).map(v => ({key, ...v})))
  )
...
export const full_export = (a: Text, b: Text) => csv(table(full(a, b), ['key', 'word', 'a', 'b']))

export const small_export = (v: VectorPair) => csv(table(flat(v), ['word', 'a', 'b']))
