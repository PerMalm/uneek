...
export const Textarea = ({store, ...props}: {store: Store<string>} & React.TextareaHTMLAttributes<HTMLTextAreaElement>) =>
  <textarea {...props}
    value={store.get()}
    onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => store.set(e.target.value)}/>

export const Checkbox = ({store, ...props}: {store: Store<boolean>} & React.InputHTMLAttributes<HTMLInputElement>) =>
...
export const Radio = ({store, ...props}: {store: Store<boolean>} & React.InputHTMLAttributes<HTMLInputElement>) =>
...
export const App = (store: Store<State>) => {
    ...
    <div className="modal-dialog" role="document">
      <div className="modal-content">
        <div className="modal-header">
          <h5 className="modal-title" id="exampleModalLabel">Use reference below.</h5>
  ...
  <React.Fragment key="below">
    <div key={1} className="row container w100" style={{paddingRight:0}}>
          ...
          <div className="panel-body">
            ...
            <h4>Input format</h4>
              <div className="radio-inline">
                 <label><Radio store={store.at('xml_input').via(Lens.iso(x => !x, x => !x))}/>txt</label>
              </div>
              <div className="radio-inline">
                 <label><Radio store={store.at('xml_input')}/>xml</label>
              </div>
            <hr/>
      ...
      <div className="col-md-4">
        <h4 className="mrgn-tp-0 text-success"><span className="glyphicon glyphicon-info-sign"></span> Suggestions</h4>
        <ul>
      ...
      <ul>
        ...
        <li>(and soon, deep analysis)  </li>
          ...
          <div>
            <hr/>
                <button className="btn btn-customi"
                  onClick={() => {
                    const {A, B} = currently()
                    Utils.download(Uneek.full_export(A, B), 'results.csv')
                  }}>Download Results</button>
              <button className="btn btn-customii"
                  onClick={() => {
                    store.at("a").set(''),
                    store.at("b").set('')
  ...
  <footer>
    <div className="inner-footer">
      <div className="container">
        <div className="row">
          ...
          <h3 className="link"> <img alt="Logga" src={logo} height='100px'/></h3>
          </div>
          ...
          <div className="col-md-4 f-contact">
            ...
            <p><a href="mailto:per.malm@nordiska.uu.se?subject=Sent%20from%20Uneek%20site:">
              <i className="fa fa-envelope"></i> <span className="glyphicon glyphicon-envelope"></span>
              Per Malm</a></p>
          </div>
        </div>
      </div>
    ...
    <div className="last-div">
      <div className="container">
        <div className="row">
          <div className="copyright">© 2018 Per Malm |
            <a target="_blank" rel="nofollow" href="https://opensource.org/licenses/MIT">MIT-license</a>
          </div>
        </div>
      </div>
    </div>
  ...
  function main() {
    ...
    const count =
      ...
      <div style={{overflow: 'hidden'}} className='thumbnail equal-width'>
        <div className='rows h100 centered vcentered'>
          <div className='caption'><h3>{name}</h3></div>
          <div className='whitebox h100 w100 scroll'>
            ...
            {Uneek.flat(w).map(({a, b, word}) =>
              <div key={word} className='cols'>
                ...
                <span className='w60 c'>{word} {a > 0 && b > 0 && <span className='small'>{a + b}</span>}</span>
    ...
    const input =
      ...
      <div className='w20 thumbnail'>
        <div className='rows h100 centered vcentered'>
          <div className='caption'><h3>Field {name.toUpperCase()}</h3></div>
          ...
          <button
            ...
            onClick={() => store.at(name).set("")}>Clear</button>
              <p/>
         </div>
       </div>
     </div>
      ...
      <div key="main" className='cols w100 h100 container col-md-4 marginalized some-height'>
        ...
        <div className='w60 rows h100'>
          {store.at('xml_input').get() &&
            <div className='cols centered marginalized'
              style={{display:'inline-block','textAlign':'center'}}>
              {keys.filter(k => store.at('show_syntax').get() || !k.startsWith('syn.shallow.')).map(
                k =>
                  <button key={k}
                    onClick={() => store.at('key').set(k)}
                    disabled={k === state.key}>k</button>)}
             </div>
          }
  ...
  return () => <div className='rows h100'>
    {above}
