...
},{"./vnode":38,"./is":39}],42:[function(require,module,exports) {
"use strict";
...
},{"./h":41}],37:[function(require,module,exports) {
"use strict";
...
},{"./vnode":38,"./is":39,"./htmldomapi":40,"./h":41,"./thunk":42}],44:[function(require,module,exports) {
"use strict";
...
exports.default = h;
//# sourceMappingURL=h.js.map
},{"./vnode":44,"./is":45}],46:[function(require,module,exports) {
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
...
var Store = /** @class */ (function () {
    function Store(transact, listen, _get, _set) {
    ...
    Store.init = function (s0) {
        ...
        return new Store(transact, function (k) { return listeners.push(k); }, function () { return s; }, set);
    };
    ...
    */
    ...
    Returns itself. */
    Store.prototype.set = function (s) {
        this._set(s);
        return this;
    };
    ...
    Returns itself. */
    Store.prototype.update = function (parts) {
        var _this = this;
        var keys = Object.keys(parts);
        this.transact(function () {
            keys.forEach(function (k) { return _this.at(k).set(parts[k]); });
        });
        return this;
    };
    ...
    Returns itself. */
    Store.prototype.modify = function (f) {
        this.set(f(this.get()));
        return this;
    };
    ...
    */
    Store.prototype.on = function (k) {
        var _this = this;
        return this.listen(function () { return k(_this.get()); });
    };
    ...
    Store.prototype.ondiff = function (k) {
        var old_value = this.get();
        return this.on(function (new_value) {
    ...
    */
    Store.prototype.transaction = function (m) {
        var a;
        this.transact(function () {
            a = m();
        });
        return a; // unsafe cast, but safe because transact will run m (exactly once)
    };
    ...
    */
    Store.prototype.via = function (lens) {
        var _this = this;
        return new Store(this.transact, this.listen, function () { return lens.get(_this.get()); }, function (t) { return _this.set(lens.set(_this.get(), t)); });
    ...
    Store.prototype.pick = function () {
        var ks = [];
        for (var _i = 0; _i < arguments.length; _i++) {
            ks[_i] = arguments[_i];
        }
        return this.via(Lens.pick.apply(Lens, ks));
    };
    ...
    */
    Store.prototype.omit = function () {
        var ks = [];
        for (var _i = 0; _i < arguments.length; _i++) {
            ks[_i] = arguments[_i];
        }
        return this.via(Lens.omit.apply(Lens, ks));
    };
    ...
    Store.prototype.relabel = function (stores) {
        var _this = this;
        var keys = Object.keys(stores);
        return new Store(this.transact, this.listen, function () {
            var ret = {};
    ...
    Note: must not use the same part of the store several times. */
    Store.prototype.merge = function (other) {
        var _this = this;
        var other_keys = {};
        Object.keys(other.get()).forEach(function (k) { return other_keys[k] = true; });
        return new Store(this.transact, this.listen, function () { return (__assign({}, _this.get(), other.get())); }, function (t) {
            _this.transact(function () {
    ...
    (static method) */
    Store.arr = function (store, k) {
        return function () {
            var args = [];
            for (var _i = 0; _i < arguments.length; _i++) {
                args[_i] = arguments[_i];
            }
            var xs = store.get().slice();
            var ret = xs[k].apply(xs, args);
            store.set(xs);
            return ret;
        };
    };
    ...
    Store.each = function (store) {
        return store.get().map(function (_, i) { return store.via(Lens.index(i)); });
    };
    /** Connect with local storage */
    Store.prototype.storage_connect = function (key, audit, api) {
        if (key === void 0) { key = 'state'; }
        if (audit === void 0) { audit = function () { return true; }; }
        if (api === void 0) { api = {
            get: window.localStorage.getItem.bind(window.localStorage),
            set: window.localStorage.setItem.bind(window.localStorage)
        }; }
        var stored = api.get(key);
        if (stored) {
            try {
        ...
        return this.on(function (s) { return api.set(key, JSON.stringify(s)); });
    };
    /** Connect with window.location.hash */
    Store.prototype.location_connect = function (to_hash, from_hash, api) {
        if (api === void 0) { api = {
            get: function () { return window.location.hash; },
            set: function (s) { window.location.hash = s; },
            on: function (cb) { window.onhashchange = cb; }
        }; }
        var self = false;
        function update() {
            if (!self) {
                var updated = from_hash(api.get());
...
(function (Lens) {
    ...
    Note: must not use the same part of the store several times. */
    function relabel(lenses) {
        var keys = Object.keys(lenses);
    ...
    Lens.relabel = relabel;
    /** Lens to a key in a record
  
    Note: the key must always be present. */
    function at(k) {
        return lens(function (s) { return s[k]; }, function (s, v) {
            return (__assign({}, s, (_a = {}, _a[k] = v, _a)));
            var _a;
        });
    ...
    function iso(f, g) {
        return lens(f, function (_s, t) { return g(t); });
    }
    Lens.iso = iso;
    ...
    function pick() {
        ...
        var lenses = {};
        keys.forEach(function (k) { return lenses[k] = at(k); });
    ...
    function key(k) {
        return lens(function (x) { return x[k]; }, function (s, v) {
            // as string: safe cast
    ...
    */
    function def(missing) {
        return iso(function (a) { return a === undefined ? missing : a; }, function (a) { return a === missing ? undefined : a; });
    }
    Lens.def = def;
    /** Compose two lenses sequentially */
    function seq(lens1, lens2) {
        return lens(function (s) { return lens2.get(lens1.get(s)); }, function (s, u) { return lens1.set(s, lens2.set(lens1.get(s), u)); });
    }
    Lens.seq = seq;
    ...
    function omit() {
        ...
        var curry = function (s) {
            var all_keys = Object.keys(s);
            var picked = all_keys.filter(function (x) { return -1 == ks.indexOf(x); });
            return Lens.pick.apply(Lens, picked);
        };
        return Lens.lens(function (s) { return curry(s).get(s); }, function (s, t) { return curry(s).set(s, t); });
    }
    Lens.omit = omit;
    ...
    function index(i) {
        var within = function (xs) {
            if (i < 0 || i >= xs.length) {
                throw 'Out of bounds';
            }
        };
        return lens(function (xs) { return (within(xs), xs[i]); }, function (xs, x) {
            within(xs);
            var ys = xs.slice();
            ys[i] = x;
            return ys;
...
(function (Undo) {
    ...
    function advance_to(s) {
        return function (h) { return Lens.at('now').set(advance(h), s); };
    }
    Undo.advance_to = advance_to;
...
})(Undo = exports.Undo || (exports.Undo = {}));
/** List with iteration and O(1) push and remove */
function ListWithRemove() {
    var dict = {};
    var order = [];
    var next_unique = 0;
    var dirty = false;
    return {
        ...
        iter: function (f) {
            if (dirty) {
                order = order.filter(function (id) { return id in dict; });
                dirty = false;
            }
...
(function (Requests) {
    ...
    function request_maker(store) {
        return Store.arr(store, 'push');
    }
    Requests.request_maker = request_maker;
    /** Make a request */
    function request(store, request) {
    ...
    function process_requests(store, process) {
        return store.ondiff(function (requests) {
    ...
    Requests.process_requests = process_requests;
})(Requests = exports.Requests || (exports.Requests = {}));

},{"process":23}],19:[function(require,module,exports) {
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
...
var reactive_lens_1 = require("reactive-lens");
function connect(patcher, store, setup_view) {
    var view = setup_view(store);
    function redraw() {
        store.transaction(function () {
            patcher(view());
        });
    }
    var off = store.on(redraw);
    redraw();
    return off;
}
function setup(patch, root) {
    while (root.lastChild) {
        root.removeChild(root.lastChild);
    }
    var container = document.createElement('div');
    root.appendChild(container);
    var vnode = patch(container, snabbdom.h('div'));
    return function (new_vnode) { vnode = patch(vnode, new_vnode); };
}
var full_patch = snabbdom.init([
    style_1.default,
...
function attach(root, init_state, setup_view, patch) {
    if (patch === void 0) { patch = full_patch; }
    var patcher = setup(patch, root);
    var store = reactive_lens_1.Store.init(init_state);
    var off = connect(patcher, store, setup_view);
    return function (setup_next_view) {
        off();
        store = reactive_lens_1.Store.init(store.get());
...
*/
function tag(tag_name_classes_id) {
    var tag_data = [];
    for (var _i = 1; _i < arguments.length; _i++) {
        tag_data[_i - 1] = arguments[_i];
    }
    var children = [];
    var key = undefined;
    var props = {};
    ...
    var hook = {};
    var tag_name = 'div';
    var matches = tag_name_classes_id.match(/([.#]?[^.#\s]+)/g);
        ...
        if (b instanceof Array) {
            ...
            switch (b.type) {
                case 0 /* Key */:
                    ...
                    break;
                case 1 /* Props */:
                    imprint(props, b.data);
                    break;
                case 2 /* Attrs */:
                    imprint(attrs, b.data);
                    break;
                case 3 /* Classes */:
                    imprint(classes, b.data);
                    break;
                case 4 /* Style */:
                    imprint(style, b.data);
                    break;
                case 5 /* Dataset */:
                    imprint(dataset, b.data);
                    break;
                case 6 /* On */:
                    imprint(on, b.data);
                    break;
    ...
    var data = { props: props, attrs: attrs, class: classes, style: style, dataset: dataset, on: on, hook: hook, key: key };
    return exports.h(tag_name, data, children);
}
exports.tag = tag;
var s;
(function (s) {
    ...
    */
    function id(id) {
        return { type: 2 /* Attrs */, data: { id: id } };
    }
    s.id = id;
    /**
    Set some classes
    ...
    */
    function classes(classes) {
        return { type: 3 /* Classes */, data: classes };
    }
    s.classes = classes;
    /**
    ...
    */
    function classed() {
        ...
        var d = {};
        (classnames.join(' ').match(/\S+/g) || []).forEach(function (name) { return d[name] = true; });
        return classes(d);
    }
    s.classed = classed;
    /**
    ...
    */
    function style(styles) {
        return { type: 4 /* Style */, data: styles };
    }
    s.style = style;
    /** Alias for style */
    s.css = style;
    ...
    */
    function attrs(attrs) {
        return { type: 2 /* Attrs */, data: attrs };
    }
    s.attrs = attrs;
    /**
    ...
    */
    function on(event_name) {
        return function (h) {
            return ({ type: 6 /* On */, data: (_a = {}, _a[event_name] = h, _a) });
            var _a;
        };
    }
    s.on = on;
    ...
    */
    function hook(hook_name) {
        return function (h) {
            return ({ type: 7 /* Hook */, data: (_a = {}, _a[hook_name] = h, _a) });
            var _a;
        };
    }
    s.hook = hook;
    ...
    */
    function props(props) {
        return { type: 1 /* Props */, data: props };
    }
    s.props = props;
    /**
    Set data attribute
    ...
    */
    function dataset(dataset) {
        return { type: 5 /* Dataset */, data: dataset };
    }
    s.dataset = dataset;
    function input(store, onEnter) {
        var tag_data = [];
        for (var _i = 2; _i < arguments.length; _i++) {
            tag_data[_i - 2] = arguments[_i];
        }
        return tag.apply(void 0, ['input',
            s.props({ value: store.get() }),
            s.on('input')(function (e) { return store.set(e.target.value); }),
            onEnter && s.on('keydown')(function (e) { return e.key == 'Enter' && onEnter(); })].concat(tag_data));
    }
    s.input = input;
    function checkbox(store) {
        var tag_data = [];
        for (var _i = 1; _i < arguments.length; _i++) {
            tag_data[_i - 1] = arguments[_i];
        }
        return tag.apply(void 0, ['input',
            ...
            s.on('input')(function (e) { return store.set(e.target.value == 'true'); })].concat(tag_data));
    }
    s.checkbox = checkbox;
    function button(label, onClick) {
        ...
        return tag.apply(void 0, ['input',
            ...
            s.on('click')(onClick)].concat(tag_data));
    }
    s.button = button;
    function textarea(store, rows, cols) {
        if (rows === void 0) { rows = 10; }
        if (cols === void 0) { cols = 80; }
        var tag_data = [];
        for (var _i = 3; _i < arguments.length; _i++) {
            tag_data[_i - 3] = arguments[_i];
        }
        return tag.apply(void 0, ['textarea',
            s.props({ value: store.get() }),
            s.attrs({ rows: rows, cols: cols }),
            s.on('input')(function (e) { return store.set(e.target.value); })].concat(tag_data));
    }
    s.textarea = textarea;
    function select(stored, keys, option) {
        var tag_data = [];
        for (var _i = 3; _i < arguments.length; _i++) {
            tag_data[_i - 3] = arguments[_i];
        }
        var off = undefined;
        return tag.apply(void 0, ['select',
            s.hook('insert')(function (vn) {
                console.log('inserted select');
                off = stored.ondiff(function (current) {
                    if (vn.elm) {
                        var i = keys.get().indexOf(current);
                        vn.elm.selectedIndex = i;
                    }
                });
            }),
            s.hook('destroy')(function () { return off && off(); }),
            keys.get().map(option),
...
function imprint(base, more) {
    for (var k in more) {
        base[k] = more[k];
    }
}
/** Convenience reexport of `snabbdom`'s `h` */
exports.h = snabbdom_h.h;
/** Common tags
...
var tags;
(function (tags) {
    tags.factory = function (name) { return function () {
        var tag_data = [];
        for (var _i = 0; _i < arguments.length; _i++) {
            tag_data[_i] = arguments[_i];
        }
        return tag.apply(void 0, [name].concat(tag_data));
    }; };
    tags.div = tags.factory('div');
    tags.span = tags.factory('span');
    tags.p = tags.factory('p');
    tags.pre = tags.factory('pre');
    tags.em = tags.factory('em');
    tags.strong = tags.factory('strong');
    tags.b = tags.factory('b');
    tags.i = tags.factory('i');
    tags.u = tags.factory('u');
    tags.strike = tags.factory('strike');
    tags.small = tags.factory('small');
    tags.table = tags.factory('table');
    tags.tbody = tags.factory('tbody');
    tags.thead = tags.factory('thead');
    tags.tfoot = tags.factory('tfoot');
    tags.tr = tags.factory('tr');
    tags.td = tags.factory('td');
    tags.th = tags.factory('th');
    tags.h1 = tags.factory('h1');
    tags.h2 = tags.factory('h2');
    tags.h3 = tags.factory('h3');
    tags.h4 = tags.factory('h4');
    tags.h5 = tags.factory('h5');
    tags.h6 = tags.factory('h6');
    tags.li = tags.factory('li');
    tags.ul = tags.factory('ul');
    tags.ol = tags.factory('ol');
    tags.option = tags.factory('option');
})(tags = exports.tags || (exports.tags = {}));

},{"snabbdom":37,"snabbdom/h":43,"snabbdom/modules/style":46,"snabbdom/modules/eventlisteners":47,"snabbdom/modules/class":50,"snabbdom/modules/props":49,"snabbdom/modules/dataset":51,"snabbdom/modules/attributes":48,"snabbdom/vnode":44,"reactive-lens":22}],7:[function(require,module,exports) {
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
...
const snabbis_1 = require("snabbis");
exports.checked = (store) => ({ kind: 'StoreSplice', selector: 'checked', store });
exports.value = (store) => ({ kind: 'StoreSplice', selector: 'value', store });
exports.attr = (attr, value) => ({ kind: 'AttrSplice', attr, value });
function html(data_name = 'snabbis-unique') {
    let next_unique = 0;
    return function (snippets, ...syncs) {
        let innerHTML = '';
        const cbs = [];
        syncs.forEach((sync, i) => {
            if (typeof sync == 'string') {
                ...
                cbs.push((elm) => window.requestAnimationFrame(() => {
                    const elms = Array.from(elm.querySelectorAll(`[${data_selector}]`));
                    elms.forEach((e) => {
                        if (sync.kind == 'StoreSplice') {
                            const { store, selector } = sync;
                            if (e[selector] != store.get()) {
                                e[selector] = store.get();
                            }
                            store.ondiff((z, a) => z !== a && (e[selector] = z));
                            e.onchange = () => store.set(e[selector]);
                            e.oninput = () => store.set(e[selector]);
                        }
                        else {
        ...
        return snabbis_1.tag('div', snabbis_1.s.key(++next_unique), snabbis_1.s.props({ innerHTML }), snabbis_1.s.hook('insert')(vn => vn.elm && cbs.forEach(k => k(vn.elm))), snabbis_1.s.hook('update')((_, vn) => vn.elm && cbs.forEach(k => k(vn.elm))));
    };
}
exports.html = html;
function flatten(xs) {
...
},{"snabbis":19}],24:[function(require,module,exports) {
...
exports.flat = (v) => Utils.record_traverse(v, ({ a, b }, occurrences) => ({ a, b, occurrences }))
    .sort((l, r) => (r.a + r.b) - (l.a + l.b));
exports.full = (a, b) => Utils.flatten(Utils.keysof(a, b)
    .sort()
    .map(key => exports.flat(exports.VectorPair(a[key] || {}, b[key] || {}))
    .map(v => (Object.assign({ key }, v)))));
...
exports.full_export = (a, b) => exports.csv(table(exports.full(a, b), ['key', 'occurrences', 'a', 'b']));
exports.small_export = (v) => exports.csv(table(exports.flat(v), ['occurrences', 'a', 'b']));

},{"./Utils":7,"comma-separated-values":24}],8:[function(require,module,exports) {
module.exports="/dist/754121c4924d7dc7b9db6189b2eea33b.svg";
},{}],9:[function(require,module,exports) {
module.exports="/dist/8a9a6344dcded81e3fb9e17d01528626.svg";
},{}],10:[function(require,module,exports) {
module.exports="/dist/82bf1b2c70de46e6dcb7bf7015e8be6b.svg";
...
},{"./../fonts/glyphicons-halflings-regular.eot":["28922bc4960078e50493a6b073bfca1f.eot",14],"./../fonts/glyphicons-halflings-regular.woff2":["5ea48df9a2a89701ddafb5ca4513fe1c.woff2",15],"./../fonts/glyphicons-halflings-regular.woff":["565245718b7921e83772586535c3692a.woff",16],"./../fonts/glyphicons-halflings-regular.ttf":["49e24543adc494df8016398aaaf7907b.ttf",17],"./../fonts/glyphicons-halflings-regular.svg":["ef2794ea219db386a0a8f0866748c084.svg",18],"_css_loader":5}],12:[function(require,module,exports) {

        var reloadCSS = require('_css_loader');
        module.hot.dispose(reloadCSS);
        module.hot.accept(reloadCSS);
      
},{"_css_loader":5}],20:[function(require,module,exports) {
...
},{"process":23}],25:[function(require,module,exports) {
/* ========================================================================
...
require('../../js/tab.js')
require('../../js/affix.js')
},{"../../js/transition.js":25,"../../js/alert.js":26,"../../js/button.js":27,"../../js/carousel.js":28,"../../js/collapse.js":29,"../../js/dropdown.js":30,"../../js/modal.js":31,"../../js/tooltip.js":32,"../../js/popover.js":33,"../../js/scrollspy.js":34,"../../js/tab.js":35,"../../js/affix.js":36}],4:[function(require,module,exports) {
var global = (1,eval)("this");
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
...
const snabbis_1 = require("snabbis");
const { div, span, h1 } = snabbis_1.tags;
      ...
      <div class="col-md-4">
        <div class="panel panel-default">
          <div class="panel-body">
            ...
            <div class="checkbox">
              <label>
              <input type="checkbox" ${Utils_1.checked(store.at('show_uniqueness'))}>
              uniqueness differentiation [(A–B) and (B–A)]</label>
            </div>
            <div class="checkbox">
              <label>
              <input type="checkbox" value="" ${Utils_1.checked(store.at('show_intersection'))}>
              intersection analysis  [A &#8745; B]</label>
            </div>
            <hr>
            <h4>Input format</h4>
<div class="radio-inline">
   <label><input type="radio" name="optradio" ${Utils_1.checked(store.at('xml_input').via(reactive_lens_1.Lens.iso(x => !x, x => !x)))}>txt</label>
</div>
<div class="radio-inline">
   <label><input type="radio" name="optradio" ${Utils_1.checked(store.at('xml_input'))}>xml</label>
</div>
    <!--        <hr>
            <h4>Statistics</h4>
            ...
            <label class="checkbox-inline">
              <input type="checkbox" value="" ${Utils_1.checked(store.at('show_syntax'))}>
                shallow
            </label>
         <!--   <label class="checkbox-inline">
            <input type="checkbox" value="">
...
<p></p>
  ...
  <div class="copyright" align="center">© 2018 Per Malm | <a target="_blank" rel="nofollow" href="https://opensource.org/licenses/MIT">MIT-license</a></div>
    ...
    function main() {
        ...
        const count = (name, w, filename) => snabbis_1.tag('.w20.thumbnail.equal-width', snabbis_1.tag('.cols.h100.centered.vcentered', snabbis_1.tag('caption', snabbis_1.tag('h3', name)), snabbis_1.tag('.whitebox.whcustom.scroll', Uneek.flat(w).map(({ a, b, occurrences }) => snabbis_1.tag('.rows', snabbis_1.tag('span.w20.r', a > 0 && a), snabbis_1.tag('span.w60.c', occurrences, a > 0 && b > 0 && snabbis_1.tag('span.small', ` (${a + b})`)), snabbis_1.tag('span.w20.l', b > 0 && b)))), snabbis_1.s.button('Download', () => Utils.download(Uneek.small_export(w), `${filename}.csv`), snabbis_1.s.classed('btn btn-customi'))));
        const input = (name) => snabbis_1.tag('.w20.thumbnail', snabbis_1.tag('.cols.h100.centered.vcentered', snabbis_1.tag('caption', snabbis_1.tag('h3', 'Field ' + name.toUpperCase())), snabbis_1.s.textarea(store.at(name), undefined, undefined, snabbis_1.s.classed('textarea'), snabbis_1.s.attrs({ "placeholder": exports.placeholder[name] })), 
        // the error message:
        errs[name] && snabbis_1.tag('.error', errs[name]), snabbis_1.tag('div.marginalized', snabbis_1.tag('p'), snabbis_1.tag('label.btn.btn-customi', 'Upload', snabbis_1.tag('input', snabbis_1.s.attrs({ 'type': 'file' }), snabbis_1.s.css({ display: 'none' }), snabbis_1.s.on('change')((e) => {
            const me = e.target;
        ...
        }))), snabbis_1.s.button("Clear", // label
        () => store.at(name).set(""), // atClick
        snabbis_1.s.classed('btn btn-customii')), snabbis_1.tag('p'))));
        return snabbis_1.tag('.rows.w100.h100.container.col-md-4.marginalized.some-height', snabbis_1.s.key('main'), input('a'), snabbis_1.tag('.w60.cols.h100', (store.at('xml_input').get() &&
            snabbis_1.tag('.rows.centered.marginalized', 
            // force automatically listed attribute buttons to stay within margins
            snabbis_1.s.style({ display: 'inline-block', 'text-align': 'center' }), keys.filter(k => store.at('show_syntax').get() || !k.startsWith('syn.shallow.')).map(k => snabbis_1.s.button(k, () => store.at('key').set(k), snabbis_1.s.attrs({ disabled: k === state.key }))))), ((state.key == '') || (!state.show_uniqueness && !state.show_intersection)) ?
            snabbis_1.tag('.h100.thumbnail.centered.vcentered.rows', centered_logo) :
            snabbis_1.tag('.rows.h100.marginalized.some-height.padding-b7', state.show_uniqueness && count('A-B', only((a, b) => b == 0), 'a-only'), state.show_intersection && count('A ∪ B', only((a, b) => a > 0 && b > 0), 'intersection'), state.show_uniqueness && count('B-A', only((a, b) => a == 0), 'b-only'))), input('b'));
    }
    return () => snabbis_1.tag('div.cols.h100', above, routes[store.get().location] || main(), below);
};

},{"./Uneek":6,"./Utils":7,"reactive-lens":22,"snabbis":19,"./images/nerlog.svg":8,"./images/uneekCenter.svg":9,"./images/slimlog.svg":10,"../node_modules/bootstrap/dist/css/bootstrap.min.css":11,"../node_modules/bootstrap/dist/css/bootstrap-theme.min.css":12,"jquery":20,"bootstrap":21}],3:[function(require,module,exports) {
...
},{"./App":4,"snabbis":19,"./index.css":3,"./App.ts":4}],0:[function(require,module,exports) {
var global = (1, eval)('this');
...
if (!module.bundle.parent && typeof WebSocket !== 'undefined') {
  var ws = new WebSocket('ws://' + window.location.hostname + ':59276/');
